...
RSpec.describe Buyer::UserRequestsController, type: :controller do
  describe 'booking detail status changes to cancellation_requested' do
    before (:each) do
      phase = create(:phase)
      admin = create(:admin)
    ...
    it 'when project unit status blocked' do
      ...
      user_request_params = { status: 'pending', project_unit_id: @project_unit1.id, user_id: @user.id, created_by_id: @user_id }
      ...
      expect(@project_unit1.booking_detail.status).to eq('cancellation_requested')
    end
    ...
    it 'when project unit status booked_tentative' do
      ...
      user_request_params = { status: 'pending', project_unit_id: @project_unit2.id, user_id: @user.id, resolved_by_id: nil, created_by_id: @user_id }
      expect { post :create, params: { user_request_cancellation: user_request_params, request_type: 'cancellation' } }.to change { UserRequest.count }.by(1)
      expect(@project_unit2.booking_detail.status).to eq('cancellation_requested')
    end
    ...
    it 'when project unit status booked_confirmed' do
      ...
      user_request_params = { status: 'pending', resolved_at: nil, reason_for_failure: nil, project_unit_id: @project_unit3.id, user_id: @user.id, resolved_by_id: nil, created_by_id: @user_id }
      expect { post :create, params: { user_request_cancellation: user_request_params, request_type: 'cancellation' } }.to change { UserRequest.count }.by(1)
      expect(@project_unit3.booking_detail.status).to eq('cancellation_requested')
    end
  end
end
