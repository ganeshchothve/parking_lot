class ReceiptObserver < Mongoid::Observer
  ...
  def after_create receipt
    ...
    SelldoLeadUpdater.perform_async(receipt.user_id.to_s, 'payment_done', {token_number: true}) if (receipt.payment_mode == 'offline' || (receipt.payment_mode == 'online' && receipt.success? ))
  end
