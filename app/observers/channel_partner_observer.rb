class ChannelPartnerObserver < Mongoid::Observer
  ...
  def before_save channel_partner
    ...
    if channel_partner.status_changed? && channel_partner.status == 'active'
      ...
      user = User.create!(name: channel_partner.name, email: channel_partner.email, phone: channel_partner.phone, rera_id: channel_partner.rera_id, location: channel_partner.location, password: generated_password, role: 'channel_partner')
