class UserKycObserver < Mongoid::Observer
  ...
  def after_create user_kyc
    SelldoLeadUpdater.perform_async(user_kyc.user_id.to_s, {stage: 'kyc_done'})
    ...
    if user_kyc.user.booking_portal_client.email_enabled? && template.present?
      ...
      email.sent!
      PaymentReminderWorker.perform_at(Time.now + 1.hour, user_kyc.user_id) if user.receipts.blank?
    end
