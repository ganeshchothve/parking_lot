class ProjectTowerObserver < Mongoid::Observer
  def after_create project_tower
    ...
    user = User.where(booking_portal_client_id: project.booking_portal_client_id, role: "admin").first || User.where(booking_portal_client_id: project.booking_portal_client_id, role: 'superadmin').first
