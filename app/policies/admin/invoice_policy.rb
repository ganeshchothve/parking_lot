  ...
  def permitted_attributes(params = {})
    ...
    when 'channel_partner'
      if record.status.in?(%w(draft rejected))
        attributes += [:number, :percentage_slab, :gst_slab, :comments, :event]
        attributes += [:agreement_amount] if record.manual?
      end
    when 'cp_admin'
    ...
    when 'admin'
      ...
      attributes += [:event]
    when 'billing_team'
      attributes += [:number, :gst_slab, :percentage_slab, :gst_slab, :agreement_amount] if record.status.in?(%w(draft))
