class Admin::InvoicePolicy < InvoicePolicy
  def index?
    out = false
    out = (user.role.in?(%w(admin superadmin channel_partner cp_owner billing_team cp cp_admin account_manager account_manager_head)) && enable_incentive_module?(user)) && current_client.enable_channel_partners?
    out
  end
