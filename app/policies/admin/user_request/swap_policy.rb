class Admin::UserRequest::SwapPolicy < Admin::UserRequestPolicy
  ...
  def permitted_attributes(params = {})
    attributes = super
    attributes += [:requestable_id, :requestable_type] if record.new_record? && record.status == 'pending' && %w[admin crm sales superadmin cp sales_admin company_owner channel_partner].include?(user.role)
