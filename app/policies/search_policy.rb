class SearchPolicy < ApplicationPolicy
  ...
  def show?
    ...
    if record.project_unit.present? && record.project_unit.status == 'hold'
      if record.lead_id != record.project_unit.booking_details.hold.first.lead_id
        valid = false
      end
    end
    valid
  ...
  def new?
    valid = true
    valid = valid && (record.lead_id == user.selected_lead_id) if user.buyer?
  ...
  def permitted_attributes params={}
    [:site_visit_id, :bedrooms, :carpet, :agreement_price, :all_inclusive_price, :project_unit_id, :project_tower_id, :floor, :step, :payment_schedule_template_id, :cost_sheet_template_id]
  end
end
