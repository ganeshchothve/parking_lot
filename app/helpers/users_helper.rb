module UsersHelper
  ...
  def filter_projects(client, user)
    if user.role?(:cp_owner)
      channel_partner = user.channel_partner
      Project.where(booking_portal_client_id: client.id).in(id: channel_partner.project_ids)
    else
      Project.where(booking_portal_client_id: client.id).in(id: user.project_ids)
    end
  end
end
