module UsersHelper
  ...
  def get_referral_code
    if current_user.referral_code.blank?
      ...
      text_field_tag '', current_user.referral_code, readonly: true, class: 'form-control col-6 float-right'
    end
  end

  def invite_friend_link
    if policy([:buyer, :referral]).new?
      link_to t('controller.referrals.new.link_name'), new_buyer_referral_path, class: 'btn btn-primary modal-remote-form-link', data:{ event_category: 'Section', event_action: 'Click', event_name: 'Invite Friend'}
    else
      ''
    end
  end
