class ProjectUnitMailer < ApplicationMailer
  ...
  def blocked(project_unit_id)
    ...
    mail(to: @user.email, subject: "Unit #{@project_unit.name} blocked")
  end

  def booked_tentative(project_unit_id)
    @project_unit = ProjectUnit.find(project_unit_id)
    @user = @project_unit.user
    mail(to: @user.email, subject: "Unit #{@project_unit.name} booked tentative")
  end

  def booked_confirmed(project_unit_id)
    @project_unit = ProjectUnit.find(project_unit_id)
    @user = @project_unit.user
    mail(to: @user.email, subject: "Unit #{@project_unit.name} booked confirmed")
  end
  ...
  def auto_release_on_extended(project_unit_id, auto_release_on_was)
    ...
    mail(to: @user.email, subject: "Received an extension to hold the unit")
  end
  ...
  def released(user_id, project_unit_id)
    ...
    mail(to: @user.email, subject: "Unit #{@project_unit.name} has been released")
  end
end
