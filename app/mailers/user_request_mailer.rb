class UserRequestMailer < ApplicationMailer
  def send_pending(user_request_id)
    ...
    make_bootstrap_mail(to: @user.email, cc: cc, subject: "Cancellation Requested for #{@name}")
  end
  ...
  def send_resolved(user_request_id)
    ...
    @cp = @user.manager
    cc = @cp.present? ? [@cp.email] : []
    make_bootstrap_mail(to: @user.email, cc: cc, subject: "Cancellation Request for #{@name} Resolved")
  end
  ...
  def send_swapped(user_request_id)
    ...
    @cp = @user.manager
    cc = @cp.present? ? [@cp.email] : []
    make_bootstrap_mail(to: @user.email, cc: cc, subject: "Swap request resolved for Unit: #{@project_unit.name} with new unit #{@alternate_project_unit.name}") if @project_unit.present?
  end
end
