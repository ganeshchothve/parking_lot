class NotifyAdminMailer < ApplicationMailer
  def send_channel_partner_activity user_id , channel_partner_id
    ...
    make_bootstrap_mail(to: User.where(booking_portal_client_id: @client.id).in(role: ["admin", "superadmin"]).distinct(:email), subject: "Channel Partner tried to add an existing lead .")
  end
end
