...
class BookingDetailMisReportWorker
  ...
  def perform user_id, filters = nil
    ...
    ExportMailer.notify(file_name, user.email, "Booking Detail MIS", user.id.to_s).deliver
  end
  ...
  def self.get_booking_detail_row(booking_detail)
    ...
    [
      ...
      project_unit.try(:project_name) || booking_detail.try(:project).try(:name),
      project_unit.try(:project_id).to_s || booking_detail.try(:project_id),
      lead.id.to_s,
      lead.name || 'N/A',
      lead.email || "N/A",
      lead.phone || "N/A",
