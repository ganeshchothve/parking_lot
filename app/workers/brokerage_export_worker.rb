...
class BrokerageExportWorker
  ...
  def self.get_invoice_row(invoice)
    invoice_row = [
      ...
      invoice.raised_date.present? ? invoice.raised_date.in_time_zone(user.time_zone).strftime('%d/%m/%Y') : "",
      ...
      invoice.net_amount,
      invoice.approved_date.present? ? invoice.approved_date.in_time_zone(user.time_zone).strftime('%d/%m/%Y') : "",
