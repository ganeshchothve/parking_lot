require 'spreadsheet'
class ChannelPartnerLeadDetailsExportWorker
  ...
  def perform emails
    ...
    ExportMailer.notify(file_name, emails, "Channel Partners Leads").deliver
  end
  ...
  def self.get_channel_partner_lead_row(project_unit)
    [
      project_unit.user.name,
      project_unit.user.email,
      project_unit.user.phone,
      project_unit.user.channel_partner.name,
      project_unit.user.confirmed? ? "Y" : "N",
      project_unit.name,
