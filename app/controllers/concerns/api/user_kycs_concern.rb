module Api
  module UserKycsConcern
    ...
    def check_any_user_kyc_params kyc_attributes
      ...
      errors = []
      errors << I18n.t("controller.errors.reference_id_already_exists", name1: "User kYC", name2: "#{kyc_attributes[:reference_id]}") if @lead.user_kycs.reference_resource_exists?(@crm.id, kyc_attributes[:reference_id].to_s)
      ...
      [:nri, :poa, :is_company, :existing_customer].each do |boolean_field|
        errors << I18n.t("controller.errors.boolean_value", name: "#{boolean_field}") if kyc_attributes[boolean_field].present? && !kyc_attributes[boolean_field].is_a?(Boolean)
      end
      [:number_of_units, :budget].each do |integer_field|
        errors << I18n.t("controller.errors.boolean_value", name: "#{boolean_field}") if kyc_attributes[integer_field].present? && !kyc_attributes[integer_field].is_a?(Integer)
      end
