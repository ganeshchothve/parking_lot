module Api
  module ReceiptsConcern
    ...
    def check_any_receipt_params receipt_attributes
      errors = []
      errors << I18n.t("controller.errors.reference_id_already_exists", name1: "Receipt", name2: "#{receipt_attributes[:reference_id]}") if @lead.receipts.reference_resource_exists?(@crm.id, receipt_attributes[:reference_id].to_s)
      [:issued_date, :processed_on].each do |date_field|
        begin
          ...
          errors << I18n.t("controller.errors.date_format", name:"#{date_field.to_s}")
        end
      end
      errors << I18n.t("controller.receipts.errors.payment_identifier_blank") unless receipt_attributes[:payment_identifier].present? || (controller_name == 'receipts' && action_name == 'update')
      errors << I18n.t("controller.receipts.errors.status_present") unless receipt_attributes[:status].present?
      errors << I18n.t("controller.receipts.errors.status_clearance_pending") if receipt_attributes[:status].present? && %w[clearance_pending success].exclude?( receipt_attributes[:status])
