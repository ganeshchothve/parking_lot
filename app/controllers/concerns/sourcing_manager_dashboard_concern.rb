module SourcingManagerDashboardConcern
  ...
  def cp_performance
    ...
    respond_to do |format|
      format.js
      format.csv { send_data CsvGenerator::CpPerformance.cp_performance_csv(@cps, @site_visits, @leads, @bookings) , filename: "cp_performance-#{Date.today}.csv", type: "text/csv" }
    end
  end
  ...
  def cp_status
    ...
    respond_to do |format|
      format.js
      format.csv { send_data CsvGenerator::CpStatus.cp_status_csv(@cp_managers_hash, @channel_partners_manager_status_count, @channel_partners_status_count) , filename: "cp_status-#{Date.today}.csv", type: "text/csv" }
    end
  end
end
