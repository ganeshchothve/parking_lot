class Api::V1::ChannelPartnersController < ApisController
  ...
  def create
    unless ChannelPartner.reference_resource_exists?(@crm.id, params[:channel_partner][:reference_id])
      ...
      if @channel_partner.save
        render json: {channel_partner_id: @channel_partner.id, user_id: @channel_partner.associated_user.id ,message: I18n.t("controller.channel_partners.notice.created")}, status: :created
      else
  ...
  def update
    unless ChannelPartner.reference_resource_exists?(@crm.id, params[:channel_partner][:reference_id])
      ...
      if @channel_partner.save
        render json: {channel_partner_id: @channel_partner.id, user_id: @channel_partner.associated_user.id, message: I18n.t("controller.channel_partners.notice.updated")}, status: :ok
      else
        render json: {errors: @channel_partner.errors.full_messages.uniq }, status: :unprocessable_entity
      end
    else
