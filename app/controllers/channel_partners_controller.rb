class ChannelPartnersController < ApplicationController
  ...
  def new
    ...
    @cp_id = params[:manager_id].present? ? User.filter_by_role('cp').where(id: params[:manager_id]).first.try(:id) : User.where(email: "manish.pandit@beyondwalls.com").first.try(:id)
