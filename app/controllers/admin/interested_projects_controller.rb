class Admin::InterestedProjectsController < AdminController
  ...
  def create
    ...
    respond_to do |format|
      if @interested_project.save
        format.html { redirect_to request.referer, notice: I18n.t("controller.notice.subscribed", name: "Project") }
      else
  ...
  def subscribe_projects
    if params[:interested_projects_attributes].present?
      ...
      format.json { render json: { errors:  I18n.t("controller.errors.not_selected", name: "Project")}, status: :unprocessable_entity }
    end
  end
  ...
  def update
    ...
    respond_to do |format|
      if (params.dig(:interested_project, :event).present? ? @interested_project.send("#{params.dig(:interested_project, :event)}!") : @interested_project.save)
        format.html { redirect_to admin_leads_path, notice: I18n.t("controller.notice.updated", name: "Interested Project") }
      else
