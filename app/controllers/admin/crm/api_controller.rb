class Admin::Crm::ApiController < ApplicationController
  ...
  def show_response
    ...
    @resource = @api.resource_class.constantize.where(id: params[:resource_id], booking_portal_client_id: current_client.id).first if params[:resource_id].present?
    @response = @api.execute(@resource, current_user) if @resource.present?
    if request.xhr?
      render layout: false
    else
