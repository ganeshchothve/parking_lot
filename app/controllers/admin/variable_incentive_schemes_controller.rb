class Admin::VariableIncentiveSchemesController < AdminController
  ...
  def create
    ...
    respond_to do |format|
      if @variable_incentive_scheme.save
        format.html { redirect_to admin_incentive_schemes_path, notice: I18n.t("controller.notice.created", name: "Incentive Scheme") }
  ...
  def update
    respond_to do |format|
      ...
      if @variable_incentive_scheme.update(permitted_attributes([:admin, @variable_incentive_scheme]))
        format.html { redirect_to admin_incentive_schemes_path, notice: I18n.t("controller.notice.updated", name: "Incentive Scheme") }
      else
    ...
    if request.get?
      ...
      respond_to do |format|
        if @variable_incentive_scheme.update(end_date: params.dig(:variable_incentive_scheme, :end_date))
          format.html { redirect_to admin_variable_incentive_schemes_path, notice: I18n.t("controller.notice.updated", name: "Variable Incentive Scheme") }
        else
          format.html { render :edit }
          format.json { render json: { errors: @variable_incentive_scheme.errors.full_messages.uniq }, status: :unprocessable_entity }
        end
  ...
  def set_variable_incentive_scheme
    ...
    redirect_to dashboard_path, alert: I18n.t("controller.errors.not_found", name: "Incentive Scheme") unless @variable_incentive_scheme
  end
