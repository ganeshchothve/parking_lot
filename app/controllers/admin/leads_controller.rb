class Admin::LeadsController < AdminController
  ...
  before_action :set_lead, except: %i[index new export search_by search_inventory new_kylas_associated_lead create_kylas_associated_lead new_kylas_lead create_kylas_lead bulk_job_new bulk_job_create]
    ...
    respond_to do |format|
      format.html do
        if current_project.present?
          ...
          host = @lead.project.try(:booking_portal_domains).try(:first)
        end
        if host.present?
  ...
  def authorize_resource
    if %w[new export search_by search_inventory new_kylas_associated_lead create_kylas_associated_lead new_kylas_lead create_kylas_lead bulk_job_new bulk_job_create].include?(params[:action])
