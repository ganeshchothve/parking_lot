module Kylas
    class CreateProjectCustomField < BaseService
      ...
      def dump_kylas_field_ids(response)
        ...
        client.set("kylas_custom_fields.meeting_project": ({id: response[:id]} rescue nil))
        project_cf_id = client.kylas_custom_fields.dig('meeting_project', 'id')
        if project_cf_id.present?
          ...
          if project_cf_response[:success]
            ...
            client.set("kylas_custom_fields.meeting_project": client.kylas_custom_fields.dig(:meeting_project).merge(name: data.dig(:field, :name), picklist_id: data.dig(:field, :picklist, :id)))
          end
        end
      end
    end
