module Kylas
  class UpdateMeetingCustomField < BaseService
    ...
    def meeting_custom_field_details
      response = Kylas::FetchMeetingCustomFieldDetails.new(user, custom_field_id).call
      if response[:success]
        data = response[:data].with_indifferent_access
        picklist_values = []
        picklist_values = data[:field][:picklist][:picklistValues].collect{|cf | {id: cf[:id], displayName: cf[:displayName]}  } rescue []
    ...
    def dump_custom_field_option_values(response)
      begin
        ...
        cp_user.set("kylas_custom_fields_option_id.meeting": pick_list_response[:id]) if pick_list_response.present?
      rescue => exception
