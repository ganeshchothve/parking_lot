  ...
  def self.tentative_reports(current_user, params={})
    ...
    data = Invoice.collection.aggregate([
        ...
        },
        {
          '$group': {
            '_id': '$project_id',
            'total_tentative_amount': { '$sum': '$amount'}
          }
        },
        {
            '$lookup': {
    ...
    data.each do |d|
      if project_wise_total_tentative_amount[d['city']].present?
        project_wise_total_tentative_amount[d['city']].push({ project_id: d['_id'], total_tentative_amount: d['total_tentative_amount'] })
      else
        project_wise_total_tentative_amount[d['city']] = [{ project_id: d['_id'], total_tentative_amount: d['total_tentative_amount'] }]
      end
    end
    project_wise_total_tentative_amount
  end
