class VariableIncentiveSchemeValidator < ActiveModel::Validator
  def validate(vis)
    ...
    existing_variable_schemes = VariableIncentiveScheme.nin(id: vis.id, status: 'disabled').in(project_ids: vis.project_ids).lte(start_date: vis.ends_date).gte(ends_date: vis.start_date).approved.present?
