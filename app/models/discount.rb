class Discount
  ...
  def not_overlapping
    errors.add(:base, "Token range is overlapping with other discounts") unless Discount.all.nin(id: [self.id]).blank? ||
      Discount.all.nin(id: [self.id]).pluck(:start_token_number, :end_token_number)
