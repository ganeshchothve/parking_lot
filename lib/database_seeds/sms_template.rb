...
module DatabaseSeeds
  module SmsTemplate
    def self.project_based_sms_templates_seed(project_id, client_id)
      ...
      Template::SmsTemplate.create(booking_portal_client_id: client_id, project_id: project_id,  subject_class: "Receipt", name: "daily_payments_report", content: '[<%= Date.current.strftime("%v") %>] Today you received <%= self.class.todays_payments_count %> payments') if Template::SmsTemplate.where(booking_portal_client_id: client_id, project_id: project_id, name: "daily_payments_report").blank?

      return Template::SmsTemplate.where(booking_portal_client_id: client_id, project_id: project_id).count
    end

    def self.lead_based_sms_templates_seed client_id
      Template::SmsTemplate.create(booking_portal_client_id: client_id, subject_class: "Client", name: "daily_sms_report", content: 'Blocked: <%= ProjectUnit.where(status: "blocked").count %>. Tentative: <%= ProjectUnit.where(status: "booked_tentative").count %>. Confirmed: <%= ProjectUnit.where(status: "booked_confirmed").count %>. Blocked Today: <%= ProjectUnit.where(blocked_on: Date.today).count %>.     *subject to cancellations') if Template::SmsTemplate.where(booking_portal_client_id: client_id, name: "daily_sms_report").blank?
      ...
      Template::SmsTemplate.create(booking_portal_client_id: client_id, subject_class: "User", name: "project_unit_released", content: "Dear <%= name %>, you missed out! We regret to inform you that the apartment you shortlisted has been released. Click here if you'd like to re-start the process: <%= dashboard_url %> Your cust ref id is <%= lead_id %>") if Template::SmsTemplate.where(booking_portal_client_id: client_id, name: "project_unit_released").blank?
      ...
      Template::SmsTemplate.create!(booking_portal_client_id: client_id, subject_class: "User", name: 'no_payment_hour_1', content: "Alert: Your KYC registration is not yet complete. Complete payment to lock your spot in the queue and avail unique benefits, call <%= self.booking_portal_client.support_number %> for more information.")  if ::Template::SmsTemplate.where(booking_portal_client_id: client_id, name: "no_payment_hour_1").blank?

      Template::SmsTemplate.create!(booking_portal_client_id: client_id, subject_class: "User", name: 'no_payment_day_1', content: "Reminder: Your KYC registration is not yet complete. Lock your spot in the queue and avail unique benefits, call <%= self.booking_portal_client.support_number %> for more information.")  if ::Template::SmsTemplate.where(booking_portal_client_id: client_id, name: "no_payment_day_1").blank?

      Template::SmsTemplate.create!(booking_portal_client_id: client_id, subject_class: "User", name: 'no_payment_day_3', content: "Reminder: You are one step away from booking your spot, call on <%= self.booking_portal_client.support_number %> for more information.")  if ::Template::SmsTemplate.where(booking_portal_client_id: client_id, name: "no_payment_day_3").blank?

      Template::SmsTemplate.create!(booking_portal_client_id: client_id, subject_class: "User", name: 'no_booking_day_4', content: "In case if you need a home loan, select loan preference and upload KYC so that our loan representative can help you.")  if ::Template::SmsTemplate.where(booking_portal_client_id: client_id, name: "no_booking_day_4").blank?

      Template::SmsTemplate.create!(booking_portal_client_id: client_id, subject_class: "User", name: 'no_booking_day_5', content: "Earn additional discounts at <%= current_project.name %> by completing your profile to help us know you better.")  if ::Template::SmsTemplate.where(booking_portal_client_id: client_id, name: "no_booking_day_5").blank?

      Template::SmsTemplate.create!(booking_portal_client_id: client_id, subject_class: "User", name: 'no_booking_day_6', content: "Thank for showing your preference for the loan requirement for your dream home at <%= current_project.name %>. We have a host financial partners who are offering home loans at competitive rates.")  if ::Template::SmsTemplate.where(booking_portal_client_id: client_id, name: "no_booking_day_6").blank?

      Template::SmsTemplate.create!(booking_portal_client_id: client_id, subject_class: "User", name: 'no_booking_day_7', content: "Earn additional discounts by referring your 3 friends at <%= current_project.name %>.")  if ::Template::SmsTemplate.where(booking_portal_client_id: client_id, name: "no_booking_day_7").blank?
