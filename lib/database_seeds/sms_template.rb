...
module DatabaseSeeds
  module SmsTemplate
    def self.seed client_id
      ...
      Template::SmsTemplate.create(booking_portal_client_id: client_id, subject_class: "ProjectUnit", name: "daily_reminder_for_booking_payment", content: "You have booked your spot among the privileged few in <%= project_name %>. Kindly pay the remaining balance to complete the booking process. The due date is just <%= I18n.l(auto_release_on) %>. Visit <%= user.dashboard_url %>") if Template::SmsTemplate.where(name: "daily_reminder_for_booking_payment").blank?
      ...
      Template::SmsTemplate.create(booking_portal_client_id: client_id, subject_class: "ProjectUnit", name: "project_unit_blocked", content: "Congratulations <%= user.name %>, <%= name %> has been Blocked / Tentative Booked for you for the next <%= booking_portal_client.blocking_days %> days! To own the home, you'll need to pay the pending amount of Rs. <%= pending_balance %> within these <%= booking_portal_client.blocking_days %> days. To complete the payment now, click here: <%= user.dashboard_url %>") if Template::SmsTemplate.where(name: "project_unit_blocked").blank?
      Template::SmsTemplate.create(booking_portal_client_id: client_id, subject_class: "ProjectUnit", name: "channel_partner_project_unit_blocked", content: "Dear <%= user.manager.name %>, <%= user.name %> has blocked unit <%= name %>.") if Template::SmsTemplate.where(name: "channel_partner_project_unit_blocked").blank?
      ...
      Template::SmsTemplate.create(booking_portal_client_id: client_id, subject_class: "ProjectUnit", name: "channel_partner_project_unit_booked_confirmed", content: "Dear <%= user.manager.name %>, <%= user.name %> has booked unit <%= name %>.") if Template::SmsTemplate.where(name: "channel_partner_project_unit_booked_confirmed").blank?
