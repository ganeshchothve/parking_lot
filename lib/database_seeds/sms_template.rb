module DatabaseSeeds
  module SmsTemplate
    def self.seed client_id
      ...
      Template::SmsTemplate.create(booking_portal_client_id: client_id, subject_class: "Invitation", name: "referral_invitation", content: "Dear <%= self.name %>, You are invited in <%= self.booking_portal_client.booking_portal_domains %> Please click here. Rails.application.routes.url_helpers.register_path(custom_referral_code: self.referred_by.referral_code)") if Template::SmsTemplate.where(name: "referral_invitation").blank?
