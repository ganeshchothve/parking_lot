class CurrencyConverter < ActiveSupport::NumberHelper::NumberConverter # :nodoc:
  DECIMAL_UNITS = { 0 => :unit, 1 => :ten, 2 => :hundred, 3 => :thousand, 5 => :lakh, 7 => :crore,
  ...
  def convert # :nodoc:
    @number = ActiveSupport::NumberHelper::RoundingHelper.new(options).round(number)
    @number = Float(number)
    ...
    rounded_number = ActiveSupport::NumberHelper::NumberToRoundedConverter.convert(number, options)
